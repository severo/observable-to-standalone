...
Data: [Borgar Þorsteinsson](https://bl.ocks.org/borgar/31c1e476b8e92a11d7e9), [Michael Zöllner](http://i.document.m05.de/2013/05/23/joy-divisions-unknown-pleasures-printed-in-3d/)`;
  ...
  main.variable(observer("chart")).define("chart", ["d3", "DOM", "width", "height", "data", "y", "area", "line", "xAxis"], function (d3, DOM, width, height, data, y, area, line, xAxis) {
    ...
    const serie = svg.append("g").selectAll("g").data(data).join("g").attr("transform", (d, i) => `translate(0,${y(i) + 1})`);
    serie.append("path").attr("fill", "#fff").attr("d", area);
    serie.append("path").attr("fill", "none").attr("stroke", "black").attr("d", line);
    svg.append("g").call(xAxis);
    return svg.node();
  });
  main.variable(observer("overlap")).define("overlap", function () {
    return 16;
  });
  ...
  main.variable(observer("x")).define("x", ["d3", "data", "margin", "width"], function (d3, data, margin, width) {
    return d3.scaleLinear().domain([0, data[0].length - 1]).range([margin.left, width - margin.right]);
  });
  main.variable(observer("y")).define("y", ["d3", "data", "margin", "height"], function (d3, data, margin, height) {
    return d3.scalePoint().domain(data.map((d, i) => i)).range([margin.top, height - margin.bottom]);
  });
  main.variable(observer("z")).define("z", ["d3", "data", "overlap", "y"], function (d3, data, overlap, y) {
    return d3.scaleLinear().domain([d3.min(data, d => d3.min(d)), d3.max(data, d => d3.max(d))]).range([0, -overlap * y.step()]);
  });
  main.variable(observer("xAxis")).define("xAxis", ["height", "margin", "d3", "x", "width"], function (height, margin, d3, x, width) {
    return g => g.attr("transform", `translate(0,${height - margin.bottom})`).call(d3.axisBottom(x.copy().domain([0, 92])).ticks(width / 80)).call(g => g.select(".domain").remove()).call(g => g.select(".tick:first-of-type text").append("tspan").attr("x", 10).text(" ms"));
  });
  main.variable(observer("area")).define("area", ["d3", "x", "z"], function (d3, x, z) {
    return d3.area().defined(d => !isNaN(d)).x((d, i) => x(i)).y0(0).y1(z);
  });
  main.variable(observer("line")).define("line", ["area"], function (area) {
    return area.lineY1();
  });
  main.variable(observer("data")).define("data", ["d3"], function (d3) {
    return d3.text("https://gist.githubusercontent.com/borgar/31c1e476b8e92a11d7e9/raw/0fae97dab6830ecee185a63c1cee0008f6778ff6/pulsar.csv").then(data => d3.csvParseRows(data, row => row.map(Number)));
  });
...
function requireFrom(resolver) {
  ...
  function require(name) {
    return arguments.length > 1 ? Promise.all(map$3.call(arguments, requireBase)).then(merge$1) : requireBase(name);
  }
...
function formatDate$1(date) {
  return isNaN(date) ? "Invalid Date" : isUTCMidnight(date) ? formatYear$2(date.getUTCFullYear()) + "-" + pad$2(date.getUTCMonth() + 1, 2) + "-" + pad$2(date.getUTCDate(), 2) : formatYear$2(date.getFullYear()) + "-" + pad$2(date.getMonth() + 1, 2) + "-" + pad$2(date.getDate(), 2) + "T" + pad$2(date.getHours(), 2) + ":" + pad$2(date.getMinutes(), 2) + (date.getMilliseconds() ? ":" + pad$2(date.getSeconds(), 2) + "." + pad$2(date.getMilliseconds(), 3) : date.getSeconds() ? ":" + pad$2(date.getSeconds(), 2) : "");
}
    ...
    default:
      {
        ...
        switch (toString$1.call(value)) {
          ...
          default:
            return (expand ? inspectExpanded : inspectCollapsed)(value, shallow, name);
        }
...
function element$1 (name, attributes) {
  ...
  ? document.createElementNS(namespaces$1[prefix], name) : document.createElement(name);
...
function* map$4 (iterator, transform) {
  var result,
      index = -1;
...
block.bullet = /(?:[*+-]|\d+\.)/;
block.item = /^( *)(bull) [^\n]*(?:\n(?!\1bull )[^\n]*)*/;
block.item = replace$1(block.item, 'gm')(/bull/g, block.bullet)();
block.list = replace$1(block.list)(/bull/g, block.bullet)('hr', '\\n+(?=\\1?(?:[-*_] *){3,}(?:\\n+|$))')('def', '\\n+(?=' + block.def.source + ')')();
block.blockquote = replace$1(block.blockquote)('def', block.def)();
block._tag = '(?!(?:' + 'a|em|strong|small|s|cite|q|dfn|abbr|data|time|code' + '|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo' + '|span|br|wbr|ins|del|img)\\b)\\w+(?!:/|[^\\w\\s@]*@)\\b';
block.html = replace$1(block.html)('comment', /<!--[\s\S]*?-->/)('closed', /<(tag)[\s\S]+?<\/\1>/)('closing', /<tag(?:"[^"]*"|'[^']*'|[^'">])*?>/)(/tag/g, block._tag)();
block.paragraph = replace$1(block.paragraph)('hr', block.hr)('heading', block.heading)('lheading', block.lheading)('blockquote', block.blockquote)('tag', '<' + block._tag)('def', block.def)();
/**
...
block.gfm.paragraph = replace$1(block.paragraph)('(?!', '(?!' + block.gfm.fences.source.replace('\\1', '\\2') + '|' + block.list.source.replace('\\1', '\\3') + '|')();
/**
 * GFM + Tables Block Grammar
 */

block.tables = merge$2({}, block.gfm, {
  nptable: /^ *(\S.*\|.*)\n *([-:]+ *\|[-| :]*)\n((?:.*\|.*(?:\n|$))*)\n*/,
  table: /^ *\|(.+)\n *\|( *[-:]+[-| :]*)\n((?: *\|.*(?:\n|$))*)\n*/
});
/**
...
inline.link = replace$1(inline.link)('inside', inline._inside)('href', inline._href)();
inline.reflink = replace$1(inline.reflink)('inside', inline._inside)();
/**
...
inline.gfm = merge$2({}, inline.normal, {
  ...
  url: /^(https?:\/\/[^\s<]+[^<.,:;"')\]\s])/,
  del: /^~~(?=\S)([\s\S]*?\S)~~/,
  text: replace$1(inline.text)(']|', '~]|')('|', '|https?://|')()
});
/**
...
function marked(src, opt, callback) {
  ...
  try {
    if (opt) opt = merge$2({}, marked.defaults, opt);
...
function when(time, value) {
  var now;
  return (now = timeouts.get(time = +time)) ? now.then(constant$e(value)) : (now = Date.now()) >= time ? Promise.resolve(value) : timeout$2(now, time).then(constant$e(value));
}
...
function Library(resolver) {
  ...
  Object.defineProperties(this, {
    ...
    require: {
      value: constant$e(require),
      writable: true,
      enumerable: true
    },
    resolve: {
      value: constant$e(resolve$1),
      writable: true,
      enumerable: true
    },
    svg: {
      value: constant$e(svg$2),
      writable: true,
      enumerable: true
    },
    tex: {
...
function variable_define(name, inputs, definition) {
  ...
  return variable_defineImpl.call(this, name == null ? null : name + "", inputs == null ? [] : map$5.call(inputs, this._module._resolve, this._module), typeof definition === "function" ? definition : constant$f(definition));
}
...
function variable_defineImpl(name, inputs, definition) {
  ...
  if (definition === noop$5) runtime._variables.delete(this);else runtime._variables.add(this); // Did the variable’s name change? Time to patch references!
...
function variable_import(remote, name, module) {
  ...
  return variable_defineImpl.call(this, name + "", [module._resolve(remote + "")], identity$a);
}
...
function module_copy(copy, injectByAlias, injectModule, map) {
  ...
  this._scope.forEach(function (source, name) {
    var target = new Variable(source._type, copy),
        inject;

    if (inject = injectByAlias.get(name)) {
      target.import(inject.name, inject.alias, injectModule);
    } else if (source._definition === identity$a) {
      // import!
      var sourceInput = source._inputs[0],
          sourceModule = sourceInput._module,
          targetModule = map.get(sourceModule) || sourceModule._copy(new Module(copy._runtime), none$3, null, map);
...
const customResolve = require.alias({
  'd3@5': d3
}).resolve; // Render selected notebook cells into DOM elements of this page
